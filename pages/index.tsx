...
import { Download, Wand2, Mail, Sparkles, Lock, Trash2, Plus, CheckCircle, Star } from "lucide-react";
...
const defaultResume: Resume = {
  ...
  summary: "Data-driven operator with 5+ years in D2C/e-commerce. Built automated reconciliation, optimized courier SLAs, and improved net margins by 12%.",
  skills: ["Excel/Power Query","Python/Pandas","Marketplace Ops (Amazon/Flipkart)","Inventory & SKU Mapping","Razorpay/Payments"],
  experience: [
    { id: uid(), title:"Senior Operations Manager", company:"Home Essentials Pvt. Ltd.", location:"Gurugram", start:"Jan 2022", end:"Present",
      bullets:["Automated courier status reconciliation across 6 partners; reduced RTO mismatch by 31%","Scaled order processing to 2k+/day using Unicommerce & custom scripts","Negotiated freight to save ₹9L/yr; improved OTIF by 8%"] },
    { id: uid(), title:"Ops Analyst", company:"Nimbus Logistics", location:"Bengaluru", start:"Aug 2019", end:"Dec 2021",
      bullets:["Built SKU master & ATS variance tracker; cut stockouts 22%","Led migration to e-invoicing; ensured 100% compliance"] },
  ],
...
/** Small UI helpers */
const Input = (p: React.InputHTMLAttributes<HTMLInputElement>) => <input {...p} className={`w-full rounded-xl border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500 ${p.className||""}`} />;
const TextArea = (p: React.TextareaHTMLAttributes<HTMLTextAreaElement>) => <textarea {...p} className={`w-full rounded-xl border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500 ${p.className||""}`} />;
...
export default function Home() {
  ...
  const TemplateView = useMemo(()=> selectedTemplate==="minimal" ? <TemplateMinimal resume={resume}/> :
                                   selectedTemplate==="sidebar" ? <TemplateSidebar resume={resume}/> :
                                   <TemplateElegant resume={resume}/>, [selectedTemplate,resume]);

  const selected = PRESET_TEMPLATES.find(t=>t.id===selectedTemplate)!;
  const isPremium = selected.isPremium;

  function handleDownload(){
    if (isPremium && !isPro) { if (!confirm("This is a premium template. Pay ₹10 to download? (simulation)")) return; }
    else if (!isPremium && freeCredits <= 0) { if (!confirm("You've used your free download. Pay ₹5 for this download? (simulation)")) return; }
    window.print();
  ...
  function generateLetter(){ const txt=generateCoverLetter(resume, jobTitle, company, "formal"); setLetterText(txt); setShowLetter(true); }
  function generateEmail(){ const txt=generateJobEmail(resume, jobTitle, company); setEmailText(txt); setShowEmail(true); }
        ...
        <div className="mx-auto max-w-7xl px-4 py-6">
          <header className="mb-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-sm">
              {isPro ? (<span className="inline-flex items-center gap-1 rounded-full bg-yellow-100 px-2 py-1 text-yellow-800"><Star className="h-4 w-4"/> Pro Active</span>)
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 print:hidden">
            ...
            <div className="rounded-2xl border bg-white p-4 shadow-sm lg:col-span-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2"><label className="text-xs text-gray-600">Full Name</label><Input value={resume.name} onChange={e=>setResume({...resume,name:e.target.value})}/></div>
                <div><label className="text-xs text-gray-600">Role / Headline</label><Input value={resume.role} onChange={e=>setResume({...resume,role:e.target.value})}/></div>
                <div><label className="text-xs text-gray-600">Location</label><Input value={resume.location} onChange={e=>setResume({...resume,location:e.target.value})}/></div>
                <div><label className="text-xs text-gray-600">Email</label><Input value={resume.email} onChange={e=>setResume({...resume,email:e.target.value})}/></div>
                <div><label className="text-xs text-gray-600">Phone</label><Input value={resume.phone} onChange={e=>setResume({...resume,phone:e.target.value})}/></div>
              </div>
              ...
              <div className="mt-4">
                <div className="mb-1 flex items-center justify-between"><h3 className="text-sm font-semibold">Experience</h3>
                  <button onClick={addExperience} className="inline-flex items-center gap-1 rounded-lg border px-2 py-1 text-xs hover:bg-gray-50"><Plus className="h-3 w-3"/>Add</button>
                </div>
                <div className="space-y-3">
                  {resume.experience.map((exp,idx)=>(
                    <div key={exp.id} className="rounded-xl border p-3">
                      <div className="mb-2 flex items-center justify-between text-xs text-gray-600">#{idx+1}
                        <button onClick={()=>removeExperience(exp.id)} className="text-red-600 hover:underline">Remove</button>
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        <Input placeholder="Title" value={exp.title} onChange={e=>setResume(r=>({...r,experience:r.experience.map(x=>x.id===exp.id?{...x,title:e.target.value}:x)}))}/>
                        <Input placeholder="Company" value={exp.company} onChange={e=>setResume(r=>({...r,experience:r.experience.map(x=>x.id===exp.id?{...x,company:e.target.value}:x)}))}/>
                        <Input placeholder="Start" value={exp.start} onChange={e=>setResume(r=>({...r,experience:r.experience.map(x=>x.id===exp.id?{...x,start:e.target.value}:x)}))}/>
                        <Input placeholder="End" value={exp.end} onChange={e=>setResume(r=>({...r,experience:r.experience.map(x=>x.id===exp.id?{...x,end:e.target.value}:x)}))}/>
                      </div>
                      <div className="mt-2">
              ...
              <div className="mt-4">
                <div className="mb-1 flex items-center justify-between"><h3 className="text-sm font-semibold">Education</h3>
                  <button onClick={addEducation} className="inline-flex items-center gap-1 rounded-lg border px-2 py-1 text-xs hover:bg-gray-50"><Plus className="h-3 w-3"/>Add</button>
                </div>
                <div className="space-y-3">
                  {resume.education.map((ed,idx)=>(
                    <div key={ed.id} className="rounded-xl border p-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <Input placeholder="Degree" value={ed.degree} onChange={e=>setResume(r=>({...r,education:r.education.map(x=>x.id===ed.id?{...x,degree:e.target.value}:x)}))}/>
                        <Input placeholder="School" value={ed.school} onChange={e=>setResume(r=>({...r,education:r.education.map(x=>x.id===ed.id?{...x,school:e.target.value}:x)}))}/>
                        <Input placeholder="Year" value={ed.year} onChange={e=>setResume(r=>({...r,education:r.education.map(x=>x.id===ed.id?{...x,year:e.target.value}:x)}))}/>
                        <Input placeholder="Details" value={ed.details||""} onChange={e=>setResume(r=>({...r,education:r.education.map(x=>x.id===ed.id?{...x,details:e.target.value}:x)}))}/>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="mt-4 rounded-xl bg-indigo-50 p-3">
                ...
                <div className="mt-2 flex gap-2">
                  <button onClick={generateLetter} className="inline-flex items-center gap-1 rounded-xl bg-indigo-600 px-3 py-1.5 text-sm text-white hover:bg-indigo-700"><Wand2 className="h-4 w-4"/>Generate Letter</button>
                  <button onClick={generateEmail} className="inline-flex items-center gap-1 rounded-xl border px-3 py-1.5 text-sm hover:bg-gray-50"><Mail className="h-4 w-4"/>Generate Email</button>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-2">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={handleDownload} className="inline-flex items-center gap-2 rounded-xl bg-gray-900 px-3 py-2 text-sm text-white hover:bg-black"><Download className="h-4 w-4"/>Download / Print</button>
                </div>
              </div>

              <div className="overflow-hidden rounded-2xl border bg-white shadow-sm print:m-0 print:border-0" ref={printRef}>
              ...
              <p className="mt-3 text-xs text-gray-500">Tip: Use your browser's Print to PDF. Set paper size to A4, margins: default, and enable background graphics.</p>
            </div>
          </div>
