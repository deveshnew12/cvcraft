...
/* ---------- Templates list (only Minimal here for demo) ---------- */
    ...
    <div className="mx-auto max-w-[800px] bg-white p-8 text-[12.2px] leading-relaxed text-gray-900 print:p-0">
      ...
      <section className="mt-4 grid grid-cols-3 gap-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">{resume.skills.map((s,i)=>(<span key={i} className="rounded bg-gray-100 px-2 py-1 text-xs">{s}</span>))}</div>
        </div>
      </section>
    </div>
  );
...
export default function Home() {
  ...
  /* Persistence */
  useEffect(()=>{ try{ const raw=localStorage.getItem("cvb:resume"); if(raw) setResume(JSON.parse(raw)); }catch{} },[]);
  useEffect(()=>{ try{ localStorage.setItem("cvb:resume", JSON.stringify(resume)); }catch{} },[resume]);

  /* Choose template */
  const TemplateView = useMemo(()=> <TemplateMinimal resume={resume}/>, [selectedTemplate, resume]);
